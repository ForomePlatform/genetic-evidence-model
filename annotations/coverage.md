...
> The coverage tables in this file are **generated by
> `scripts/compute_coverage.py`** from the released YAML annotations.
> The documented corpus is the four manual papers plus Duerr `v0` and
> Inouye `v1`: 28 `GeneticEvidence` items, 95 source-anchored
> assertions. (Inouye `v0` is retained in the repository for comparison
> but is not part of the documented corpus.) Earlier hand-maintained
> counts have been superseded; re-run the script after any annotation
> edit.
...
| Knockout Type          |  1 |  6 | activation fires on non-knockout model-organism items |
...
(narrower than Organism, which also fires on `IN_VIVO` method).
Only Jossin GE-3 (the conditional Llgl1 knockout) populates the
dimension. The other five applicable items are model-organism
experiments that are not knockouts (dominant-negative injections,
retinal electroporation, and so on). The low population is not a schema
