...
design. The corpus exposed therapeutic response directly through the cited
anti-p40 antibody trial in Duerr (GE-new-1). It also exposed an analogous
within-paper synthesis problem, a higher-order claim over several items from
one paper. Both issues are recorded under candidate extension CE-DU3.
...
carry evidence content, with ECO and SEPIO shown separately and the ClinGen
and ACMG/AMP frameworks combined. IAO, OBI, and PROV-O are omitted because
they are orthogonal to all ten patterns. They provide upper-level classes,
assay hierarchies, and provenance relations that the represented models may
reuse, but do not themselves represent these patterns.
...
column because they score complementary levels of evidence, gene--disease
validity and variant pathogenicity, and the column is read as their union.
\cfull{} indicates that the pattern is a scored evidence category in the
framework that operates at the pattern's level; the row note names that
framework, and the absence of a criterion in the other framework is not
counted as a gap when the pattern lies outside its level, as a gene-level
claim does for ACMG/AMP (rows 2 and 6). \chalf{} indicates partial or
indirect treatment, including a required input that is not itself scored,
or treatment by a ClinGen product other than the validity framework.
\cnone{} indicates that neither framework provides a corresponding scored
category. For GEM, \cfull{} indicates that
...
is not represented. In any column, \cna{} indicates that the pattern is
outside the standard's subject matter, so its absence is not a coverage gap.
...
\begin{table}[H]
  ...
  five standards and GEM. \cfull{} = represented natively; for ECO, a
  suitable term exists; for ClinGen/ACMG, read as the union of the two
  frameworks, a scored category in the one at the pattern's level.
  \chalf{} = represented only through the standard's extension mechanism,
  only by reference to another resource, or only in part. \cforce{} = forced
  fit recorded as a candidate extension (GEM only). \cnone{} = not
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.30\linewidth} c c c c c c >{\raggedright\arraybackslash}X@{}}
    ...
    1. Mendelian pedigree: variant co-segregates with disease in a family
      ...
      & segregation strength (meioses, LOD) is unstructured; activation scope for Variant targets (CE-DU4) \\
...
\begin{enumerate}
  ...
  \item \emph{Polygenic score.} FHIR can represent a risk-prediction study
    ...
    Inouye items are forced fits: the target type and Resolution are recorded with \dimval{Variant} as
    placeholders, and Variant Ascertainment is set to the documented sentinel. These conditions motivated CE-IN1 through
  ...
  \item \emph{Therapeutic response.} This pattern fits FHIR Evidence's
    ...
    than forced into a genetic domain. Together with the authors' therapeutic-strategy recommendation, it forms
    the second CE-DU3 instance within the same paper.
...
either require an extension mechanism because they were designed for a
different primary purpose, or provide an evidence-type vocabulary that names
the evidence without structuring the claim. Rows 6 through 8 identify
patterns that the pilot exposed as forced fits and therefore as documented
areas for extension. Rows 9 and 10 are outside GEM's intended scope, and the
matrix indicates which standards can represent them.
