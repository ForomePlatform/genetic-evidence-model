...
%
%  Embeds the machine-checkable artifacts (the SHACL schema and the six
...
% ---------------------------------------------------------------------
\section*{SA1\quad SHACL schema (\texttt{genetic\_evidence.shacl.ttl})}
\addcontentsline{toc}{section}{SA1\quad SHACL schema}
...
reference in \fpath{schema/dimensions.md} (in the repository), it
constitutes the machine-checkable representation of the model's
implemented constraints (the always-required dimensions, value sets,
source-span presence, and the activation rules for variant
ascertainment, mode of inheritance, and organism).
...
% ---------------------------------------------------------------------
\section*{SA2\quad Annotation YAML files}
\addcontentsline{toc}{section}{SA2\quad Annotation YAML files}
...
aggregate dimension-coverage counts are given in \cref{tab:supp-coverage}
(\cref{sec:supp-coverage-note}) and whose flag totals are summarized in
the main paper's evaluation section. Each annotation contains
...
Opus 4.7 (Duerr) and 4.8 (Inouye) working from the schema, the four manual
annotations, and the source PDFs; they were curator-reviewed
assertion-by-assertion.

% ---- B.1 Jossin ----
\subsection*{SA2.1\quad Jossin et al. 2017 (Llgl1)}
\addcontentsline{toc}{subsection}{SA2.1\quad Jossin et al. 2017 (Llgl1)}
...
% ---- B.2 Davis ----
\subsection*{SA2.2\quad Davis et al. 2011 (TTC21B)}
\addcontentsline{toc}{subsection}{SA2.2\quad Davis et al. 2011 (TTC21B)}
...
% ---- B.3 Nelson ----
\subsection*{SA2.3\quad Nelson et al. 1992 (CD18)}
\addcontentsline{toc}{subsection}{SA2.3\quad Nelson et al. 1992 (CD18)}
...
% ---- B.4 Gupta ----
\subsection*{SA2.4\quad Gupta et al. 2015 (ATP6AP2)}
\addcontentsline{toc}{subsection}{SA2.4\quad Gupta et al. 2015 (ATP6AP2)}
...
% ---- B.5 Duerr ----
\subsection*{SA2.5\quad Duerr et al. 2006 (IL23R)}
\addcontentsline{toc}{subsection}{SA2.5\quad Duerr et al. 2006 (IL23R)}
...
% ---- B.6 Inouye ----
\subsection*{SA2.6\quad Inouye et al. 2018 (metaGRS)}
\addcontentsline{toc}{subsection}{SA2.6\quad Inouye et al. 2018 (metaGRS)}

\noindent\textbf{Authoritative source:}\\
\url{https://github.com/ForomePlatform/genetic-evidence-model/blob/master/annotations/v1/inouye2018.yaml}

\medskip
\noindent AI-drafted annotation, curator-reviewed (\texttt{v1}
re-annotation under the current protocol; the original \texttt{v0} draft
is retained in the repository for comparison). 4 \GeneticEvidence{}
...
\lstinputlisting[language=yaml]{../annotations/v1/inouye2018.yaml}
